...
function protectedInput(text: string, position: number, token: string): boolean {
  ...
  if (token.includes('_') || /[\\/.:]/u.test(token)) return true;
...
function addCandidate(
  ...
  if (word.includes('_') || /[\\/.:]/u.test(word) || /^(?:sk|pk|ghp|github_pat|xox[baprs]-)/iu.test(word)) return null;
...
function buildModel(context: LocalSuggestionContext): LanguageModel {
...
export function suggestLocal(
  ...
  if (!settings.autocomplete || settings.language.toLowerCase() !== 'en') return [];

...
import { createSessionTempWorkspace, plifTempRoot, TEMP_WORKDIR } from '../src/temp-workspace.js';
...
test('session temp workspace creates an isolated host directory mounted at /temp and cleans it up', async () => {
  ...
  try {
    ...
    assert.equal(path.dirname(path.dirname(workspace.hostPath)), path.resolve(plifTempRoot()));

...
function intArg(name, fallback, { min = 1, max = Number.MAX_SAFE_INTEGER } = {}) {
  ...
  if (!Number.isInteger(value) || value < min || value > max) {
    process.stderr.write(`[DME Spyx] invalid ${name}: ${raw}\n`);
    process.exit(2);
  }
  return value;
}
...
function validateCapsule(capsule) {
  ...
  if (capsule.schema !== "dme-spyx-capsule/v1") {
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", async () => {
    ...
    try {
      ...
      process.stderr.write(`[DME Spyx] write failed: ${error?.stack || error}\n`);
      send(res, 500, { ok: false, error: "write-failed" }, origin);
    }
  });

  req.on("error", (error) => {
    process.stderr.write(`[DME Spyx] request error: ${error?.message || error}\n`);
  });
});

server.on("clientError", (error, socket) => {
  process.stderr.write(`[DME Spyx] client error: ${error?.message || error}\n`);
  if (socket.writable) socket.end("HTTP/1.1 400 Bad Request\r\n\r\n");
});

function shutdown(signal) {
  process.stdout.write(`[DME Spyx] ${signal}; shutting down\n`);
...
server.listen(PORT, HOST, () => {
  ...
  process.stdout.write(`[DME Spyx] max payload: ${MAX_BYTES} bytes\n`);
  if (EXTENSION_ID) {
    process.stdout.write(`[DME Spyx] restricted extension id: ${EXTENSION_ID}\n`);
  }
});

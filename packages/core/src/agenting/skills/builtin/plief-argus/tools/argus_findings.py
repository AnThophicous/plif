...
def _cycles(record):
    ...
    (core/findings.md rule 1: regressions reopen WITH CAUSE). Each entry of
...
def posture(dimensions, coverage_pct):
    """dimensions: {name: level}. Deterministic table from core/posture.md."""
    if coverage_pct < 70:
...
if __name__ == "__main__":
    ...
    if args.mode == "check":
        errs = check(data if isinstance(data, list) else data.get("findings", []))
        print("\n".join(errs) if errs else "OK: lifecycle valid")
        sys.exit(1 if errs else 0)
    if args.mode == "dedupe":
        merged, dups = dedupe(data if isinstance(data, list)

...
(() => {
  ...
  async function buildPliefCapsule() {
  ...
  function downloadPliefCapture(capsule) {
    ...
    a.href = u;
    a.download = slug + '.plief-capture.json';
    a.click();
  ...
  function ensureUI() {
    ...
    btn.textContent = '🛰 Pli'ef Capture';
    btn.title = 'Pli'ef Capture: capturar componente localmente';
    ...
    panel.innerHTML = `
      <div style="font-weight:700;margin-bottom:4px;">21st∞ — Pli'ef Capture Bridge</div>
      ...
      <button id="t21st-plief" style="width:100%;cursor:pointer;border:1px solid #0f172a;background:#eef2ff;color:#0f172a;border-radius:8px;padding:9px 12px;font-size:13px;font-weight:650;margin-bottom:6px;">🛰 Enviar para Pli'ef Capture</button>
      ...
      <div id="t21st-status" style="color:#64748b;font-size:11px;line-height:1.5;border-top:1px solid #e2e8f0;padding-top:8px;">Pli'ef Capture pronto — captura local + handoff autorizado.</div>
    `;
    ...
    panel.querySelector('#t21st-plief').addEventListener('click', async () => {
      const btn = panel.querySelector('#t21st-plief');
      btn.disabled = true;
      const old = btn.textContent;
      btn.textContent = '🛰 Preparando capsule…';
      try {
        const capsule = await buildPliefCapsule();
        const resp = await new Promise((resolve) => {
          let done = false;
          chrome.runtime.sendMessage({ type: 'plief-ingest', capsule }, (r) => {
            if (done) return;
            done = true;
        ...
        if (resp && resp.ok) {
          flash('🛰 Enviado ao Pli'ef Capture' + (resp.stored ? ' · ' + resp.stored : '') + '.');
          try {
            chrome.storage.local.get({ t21stLog: [] }, ({ t21stLog }) => {
              t21stLog.unshift({
                ts: new Date().toLocaleTimeString(),
                name: '[PLIEF] ' + (state.info.name || state.info.slug || 'component'),
              });
              chrome.storage.local.set({ t21stLog: t21stLog.slice(0, 20) });
            });
          } catch {}
        } else {
          downloadPliefCapture(capsule);
          flash('Bridge offline — capsule .plief-capture.json baixado.');
        }
      } catch (e) {
        flash('Pli'ef Capture falhou: ' + String(e && e.message || e));
      } finally {
        btn.disabled = false;
        btn.textContent = old;
      }
  ...
  function flash(msg) {
    ...
    flashTimer = setTimeout(() => {
      s.textContent = 'Pli'ef Capture pronto — captura local + handoff autorizado.';
    }, 4000);
  }
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d) {
          ...
          if (el) el.textContent = `Plano oficial: ${d.signedIn ? 'logado' : 'anônimo'} · ${d.used || 0}/${d.limit || 0} cópias/dia${d.isMember ? ' · membro' : ''} · Pli'ef Capture disponível.`;
        }
      }
    } catch {}

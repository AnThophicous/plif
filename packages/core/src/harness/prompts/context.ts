...
export const historicalContextModule = definePromptModule({
  ...
  order: 92,
  enabled: (context) =>
    Boolean(context.guidance?.briefing.trim()) ||
    Boolean(context.memory?.trim()) ||
    Boolean(context.notes?.trim()),
  render: (context) => {
    const blocks: string[] = ['# Historical workspace context'];
    if (context.guidance?.briefing.trim()) {
      blocks.push(
        'Learned guidance is fallible. Treat candidate and contested guidance as a',
        'hypothesis to verify; current repository evidence always wins.',
        quotedBlock('LEARNED_GUIDANCE', context.guidance.briefing),
      );
    }
    if (context.memory?.trim()) {
      blocks.push(
        'Memory summarizes earlier sessions and may be stale or incomplete.',
        quotedBlock('SESSION_MEMORY', context.memory),
      );
    }
    if (context.notes?.trim()) {
      blocks.push('Earlier agent notes:', quotedBlock('AGENT_NOTES', context.notes));
    }
    return blocks.join('\n\n');
  },
});
...
export function quotedBlock(label: string, source: string): string {
  ...
  const escaped = source.trim().replaceAll(start, '[escaped begin marker]').replaceAll(
    end,
    '[escaped end marker]',
  );
  return `${start}\n${escaped}\n${end}`;

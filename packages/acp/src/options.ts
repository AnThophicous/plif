 ...
 * picker. Mirrors the CLI's model-listing logic (provider access map +
 * catalog selection) so the AionUi model selector reflects what plif can
 * actually reach, and model switches persist exactly like `plif model set`.
 */
...
export const MODES: ReadonlyArray<{ id: PlifMode; name: string; description: string }> = [
  ...
  },
  {
    ...
    description: 'Auto-accepts everything — no permission prompts',
  },
];
...
// ── Provider access (reimplemented from @plif/cli) ─────────────────────
...
async function buildAccessMap(
  ...
  for (const provider of catalogProviders(stored)) {
    // ChatGPT/Codex is an optional local session, not a generic key provider.
...
/** The picker shows ranked, reachable models; the current one is always present. */
...
/**
 * Persist a model choice exactly like `plif model set`: validate first, then
 * write preset+model into the stored config. The adapter rebuild its provider
 * from the stored config every turn, so the switch applies from the next turn.
 */
export async function applyModelChoice(
  ...
  // Validate before persisting so a bad pick never corrupts the config.
  ...
  if (!check.ok) {
    throw new Error(`Cannot switch to ${modelId}: ${check.problem ?? 'invalid configuration'}${
